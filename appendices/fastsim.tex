...
\intro{Simulated event samples of various physics processes are a key ingredient within analyses for understanding the detector and the physics behind \gls{lhc} collision data. An introduction to the FastSimulation (\FSIM{}) package of \gls{cms} is given in the following. Compared to the standard \GEANT{}4-based simulation workflow, it provides a fast alternative for simulating the \gls{cms} detector response and emulating the reconstruction of events. It is tightly integrated into
the standard workflow but sidesteps some of the computing-intense standard algorithms by using various approximations. In particular, a fast emulation of the track reconstruction is performed by relying on truth information which is detailed in the following. The presented work is publish in Ref.~\cite{fastsim-mkomm}
.}
