...
\myfigure{\label{fig:technique-parton-muon}Distributions of the muon (a)~transverse momentum and (b)~pseudorapidity for $t$-channel single top quark production at $13~\TeV$ for various W~boson decay chains. The distributions have been generated using \POWHEG interfaced with \TAUOLA and \PYTHIA8.}{
...
\myfigure{\label{fig:technique-parton-cosTheta}Distributions of the polarization angle for $t$-channel single top quark production at $13~\TeV$: (a)~inclusive distribution; (b)~only events with $\pt(\mu)>20~\GeV$. The distributions have been generated using \POWHEG interfaced with \TAUOLA and \PYTHIA8.}{
...
\begin{align}
L_\mathrm{\TUNFOLD}\big(\,\vec{x},\,\lambda\,\big|~\tau\big)&=\sum_{i}\,\sum_{j}~\Big(y_{i}-\big({\mathcal{R}}\cdot\vec{x}\,\big)_{i}\Big)\cdot\mathcal{C}_{ij}^{-1}\cdot\Big(y_{j}-\big({\mathcal{R}}\cdot\vec{x}\,\big)_{j}\Big)\label{eq:technique-tunfold-response}\\
