...
is minimized in the fit instead which is however equivalent to maximizing Eq.~\ref{eq:technique-likelihood} with respect to the scale factors. Since the templates are normalized to the expected \gls{sm} cross sections times integrated luminosity, an estimated scale factor can be directly translated into a total cross section as

\begin{equation}
\sigma_\mathrm{meas.}=\frac{N_\mathrm{obs.}}{A\cdot\epsilon\cdot{\textstyle{\int}L} }\equiv\beta\cdot\underbrace{\frac{N_\mathrm{exp.}}{A\cdot\epsilon\cdot{\textstyle{\int}L} }}_\mathrm{template}=\beta\cdot\sigma_\mathrm{\gls{sm}}
\end{equation}
...
because the analysis phase space is usually only optimized to measure the signal process. Thus these constraints with uncertainties $\sigma^{(j)}$ reflect the a priori believes of the background contributions in the signal phase space where the fit is carried out. The log-normal distribution is explicitly preferred as prior constraint over e.g. Gaussian distributions because in the case of large uncertainties log-normal distributions will not be truncated when requiring that the scale factors are always positive. Such a truncation can lead to a modified mean which then biases the fit result.
