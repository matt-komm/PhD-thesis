...
\myfigure{\label{fig:technique-parton-example}Production and decay chain of an exemplary $t$-channel single-top-quark event at parton level as produced by \POWHEG{v2} interfaced with \MADSPIN and \PYTHIA{8}. Some intermediate state particles have been omitted or are not stored by the generator.}{
\includegraphics[scale=0.75]{figures/technique/massiveRadiation.pdf}
}

\myfigure{\label{fig:technique-parton-muon}Distributions of the (a)~transverse momentum and (b)~pseudorapidity of the final state lepton produced in $t$-channel single top quark production at $13~\TeV$. The lines represent various decays of W~bosons into either direct muons/taus or into muons via intermediate tau decays. The distributions have been generated using \POWHEG{v2} interfaced with \MADSPIN and \PYTHIA{8}.}{
