...
The other level at which observables are can be defined is the ``particle level''. Here, an event selection similar to the one at reconstruction level is applied on the generated particles after hadronization. This allows to measure a distribution close to the visible phase space of the detector. It is therefore also referred to as ``fiducial'' measurement. The advantage is that an extrapolation into the inclusive phase space as it is intrinsically performed for parton level measurements (e.g. inclusive cross sections) is not required. Furthermore, when defining an observable at parton level, the utilized intermediate particles stored by generators may not follow the same rules across the various programs. In the manual of the \RIVET program, where particle-level measurements can be archived and compared to predictions by generators, the following statement about parton-level outputs can be found: ''\textsl{The unfortunate truth is that most of the event record is intended for generator debugging rather than physics interpretation}''~\cite{Buckley:2010ar}. Observables at particle level are on the other hand much less affected by such potential implementation-dependent differences since only stable particle with a mean lifetime of more than $30~\mathrm{ps}$ with a few extra information are used. A comparison of parton and particle level objects is shown in Fig.~\ref{fig:technique-parton-particle}. 
...
After hadronization, quarks and gluons appear as jets consisting of hadrons and non-prompt leptons at particle level. Charged leptons may radiate photons which are accounted for at particle level by clustering close-by photons with charged leptons. These are then referred to as ``dressed'' leptons. The performed clustering enables a universal treatment of \gls{qcd}/\gls{qed} radiations without utilizing information about intermediate particles in the decay chain which is more complicated at parton level (especially in the definition of the spectator quark) and may be generator-dependent as well. Detailed definitions of the analysis objects at particle level are provided in the following.

\begin{description}
\item[Dressed leptons] Photons which do not stem from hadron decays which are clustered with prompt muons or electrons if they are within $\Delta R=\sqrt{\Delta\eta^2+\Delta\phi^2}<0.1$. A dressed lepton consists of exactly one charged lepton and any number of potential close-by photons.
...
\item[Jets] Jet are clustered from stable particles excluding the ones used in the definition of the dressed leptons and neutrino. The anti-$k_\mathrm{T}$ algorithm with a distance of $R=0.4$ is employed at 13~TeV mimicking the jet size at reconstruction level.
\item[Pseudo top quark]
\end{description}
...
The fiducial cross section can obtained from the inclusive cross section by modifying Eq.~\ref{eq:technique-xsec-measurement} as
...
where $A_\mathrm{fid.}$ denotes the acceptance of events in the fiducial phase space. Similar to the acceptance and efficiency of the event selection after reconstruction, it can be estimated from a sample of simulated signal events as $A_\mathrm{fid.}=N_\mathrm{fid.}/N_\mathrm{total}$. 
