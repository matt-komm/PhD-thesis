...
%This is however an ill-posed problem which cannot be solved in general without applying a regularization scheme as detailed in the following.
...
where the continuous distributions are converted into vectors~(histograms) and the response, acceptance, and efficiency functions are described by matrices~(two-dimensional histograms). Elements of the response matrix $\mathcal{R}_{ij}$ can be interpreted as the transition probability $p_{i\to j}$ that an event at parton or particle level occurring in bin $i$ is measured in bin $j$. An simple attempt to solve Eq.~\ref{eq:technique-folding} for $\vec{x}$ with a simple inversion of the response matrix reveals that the unfolding problem is actually ill-posed. This results into solutions with large variance and high anticorrelations between bins which can be observed as oscillations~\cite{Cowan:2002in}. Figure~\ref{fig:technique-ill-unfolding} demonstrates this for a simple model where a exemplary distribution $g(x)=\frac{1}{2}+0.44\cdot x$, similar to the expected distribution of the top-quark polarization angle at parton level, has been folded with a simple Gaussian smearing function $R(y,x)\propto(x-y)^2/(2\sigma^2)$ using a resolution of $\sigma=0.15$. Small deviations~(dash-orange line) from the folded distribution~(violet markers), which can just be a statistical fluctuation, yields an unphysical, oscillating solution.
...
\begin{tikzpicture}[baseline=(current bounding box.center),every left delimiter/.style={xshift=0.4em},every right delimiter/.style={xshift=-0.55em}]
...
\hphantom{-}1 & -2 & \hphantom{-}1 & \hphantom{-}0  \\
  & & &  \\[0.5em]
\hphantom{-}0  & \hphantom{-}1  & -2 & \hphantom{-}1 & \\
...
The regularization strength is controlled with the parameter $\tau$ and needs to be optimized as detailed below. The vector $\vec{x}_{0}$ allows to bias the calculation of the second derivatives. It is usually set to the expectation from theory. This way, the regularization vanishes when unfolding the expectation from theory for closure tests since $(\Gamma\,(\vec{x}_\mathrm{exp.}-\vec{x}_{0}))^2=0$. Thus, the result is bias-free even for heavily curved expectations like e.g. $\pt$ spectra of particles. An alternative procedure to achieve this is to reweight $\vec{x}$ as $x_{i}^\prime=x_{i}/x_{i}^\mathrm{exp.}$ which lets the regularization vanish as well when unfolding the expectation since $\Gamma\,\vec{x}^{\,\prime}_\mathrm{exp.}=0$. The last part of the loss function, Eq.~\ref{eq:technique-tunfold-efficiency}, attempts to match the overall normalization of the solution by accounting for the acceptance and reconstruction efficiencies per bin. It is minimized independently with respect to the Lagrange parameter $\lambda$.

Finding the optimal regularization strength is of crucial importance. If the applied regularization is too weak, the unfolding with become unregulated and thus leads to unphysical solutions with oscillating behavior. On the other hand, if the regularization is too strong, the solution will be biased towards the solution of minimum curvature. The solution in this case is often then just the expectation from theory since the bias vector is set to the expectation. A method to analyze the unfolding behavior as a function of the regularization strength has been developed in Ref.~\cite{Komm-thesis}. The idea is based on the calculations of the resulting bin-by-bin correlations as a function of the regularization strength. The covariance matrix is given by propagation of uncertainty

\begin{equation}
\mathcal{C}_{x}=J\cdot\mathcal{C}_{y}\cdot J^{T}=\tilde{\mathcal{R}}^{-1}\cdot\mathcal{C}_{y}\cdot\big(\tilde{\mathcal{R}}^{-1}\big)^{T}\,
\end{equation}

where $J$ denotes the Jacobian matrix. This allows to define the average correlation between bins as

\begin{equation}
\bar{\rho}_{j}\equiv\frac{1}{n-j-1}\sum_{i}^{n-j-1}\rho_{i,i+j},\qquad \rho_{i,j}=\frac{\mathcal{C}_{x,ij}}{\sqrt{\mathcal{C}_{x,ii}\cdot\mathcal{C}_{x,jj}}}\,.
\end{equation}

The global correlation

\begin{equation}
\rho_{\mathrm{global}, i}=\sqrt{1-\mathcal{C}_{x,ii}\cdot\big(\mathcal{C}_{x}^{-1}\big)_{ii}}
\end{equation}


\myfigure{\label{fig:technique-tauscan}.}{
\subfloat[]{\includegraphics[width=0.48\textwidth]{figures/technique/scanTau.pdf}}\hspace{0.03\textwidth}
\subfloat[]{\includegraphics[width=0.48\textwidth]{figures/technique/unfoldDist.pdf}}
}

 Various methods are proposed to find the optimal regularization strength $\tau$. 


 correlations, subway plot, coverage within 2s, alternative FBU
