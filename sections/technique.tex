...
where $p$ denotes the achieved purity of a selection $x_{i}>C_{i}$. Other common measures of separation are the misclassification error or the so-called ``Gini'' index~\cite{Gini}. The separation measures are constructed to be symmetric when swapping the signal and background classes since obtaining a background leaf with high purity is of equal importance. A node is not split if it contains less than a predefined minimum number of events to ensure that the decisions per node and the binary outputs per leaf are statistically significant. This also mitigates a potential ``overtraining'' of a decision tree which occurs when statistical fluctuations are learned instead of the underlying physical distributions due to the finite statistics of the training sample. Additional caution is required when using a sample which contains a portion of negatively weighted events~(e.g. generated with \MGAMC). A decision may be trained incorrectly if a tree has selected a large fraction of negatively weighted events since distributions of the selected events can contain unphysical regions where a negative event yield is predicted. To tweak the cancellation of negatively weighted events the minimum number of events can be increased further beyond the statistical motivated threshold one would choose when training only on a sample of purely positively weighted events. In addition, the values of possible working points per observable which are analyzed to find the optimal node splitting can be restricted which prevents that a decision becomes sensitive to single events close to the selection borders.
...
Here, each decision tree output is multiplied by a so-called boosting weight $w_{i}$. This way of combining multiple decision trees has another advantage. It has been shown that the majority vote can yield a classifier with high accuracy by applying a ``boosting'' procedure for which it is however only necessary that the individual decision trees are ``weak learners''~\cite{Schapire1990,FREUND1995256}. This means that they need to have a classification accuracy which is only slightly better than random guessing. The decision trees can therefore be kept very shallow, e.g. only two or three layers of selections, which also improves their robustness against overtraining. The boosting procedure accounts then for the individual accuracy per tree by adjusting its weight in the majority vote which results then into a ``strong learner''.
...
\begin{equation}
L=\prod_{i}^\mathrm{bins}~\frac{p_{i}^{\,d_{i}}\cdot e^{-p_{i}}}{d_{i}!},\qquad p_{i}=\beta^{\mathrm{(sig.)}}\cdot t^{\mathrm{(sig.)}}_{i}+\sum_{j}^\mathrm{bkgs.}~\beta^{(j)}\cdot t^{(j)}_{i}. \label{eq:technique-likelihood}
\end{equation}

Here, the amount of observed data events $d_{i}$ per bin $i$ follows a Poisson distribution assuming an expected event yield $p_{i}$ as mean. These are obtained by summing the respective contributions of the signal and background templates $t^\scriptn{(X)}_{i}$ per process $X$. The normalization per process is controlled through scale factors $\beta^\scriptn{\mathrm{(X)}}$. These are then estimated through the fit from data by maximizing the likelihood. The signal scale factor is also reffered to as signal ``strength'' whereas the background scale factors are reffered to as nuciance parameters because their fit result is of lesser importance. Technically, the \THETA framework~\cite{} is employed in which the negative logarithm of the likelihood

\begin{equation}
-\ln\Big[L\big(\vec{\beta}\big)\Big]=-\sum_{i}^\mathrm{bins}~\Big[\,d_{i}\ln p_{i}\big(\vec{\beta}\big)-p_{i}\big(\vec{\beta}\big)\Big]+\mathrm{const.}
\end{equation}

is minimized in the fit with respect to the scale factors. Since the templates are normalized to the expected \gls{sm} cross sections times luminosity, an estimated scale factor can be directly translated into a total cross section as

\begin{equation}
\sigma_\mathrm{meas.}=\frac{N_\mathrm{obs.}}{A\cdot\epsilon\cdot L}=\beta\cdot\underbrace{\frac{N_\mathrm{exp.}}{A\cdot\epsilon\cdot L}}_\mathrm{template}=\beta\cdot\sigma_\mathrm{\gls{sm}}
\end{equation}

where $A$ denotes the acceptance and $\epsilon$ the efficiency of the event selection which are estimated through the simulated templates. For the background scale factors additional constraints are added to the likelihood using log-normal priors as

\begin{equation}
-\ln(L^\prime)=-\ln(L)+\sum_{j}^\mathrm{bkgs.}~\frac{1}{2}\cdot\left(\frac{\ln\,\beta^{(j)}}{\sigma^{(j)}}\right)^{2}
\end{equation}

because the analysis phase space is usually only optimized to measure the signal process. Thus these constraints reflect the 

The log-normal distribution is preferred over e.g. Gaussian distribution because in the case of large prior widths the distributions are not truncated. Truncation can result in a modified mean which then biases the result.

barlow-beeston method, sys
