...
the \GRADIENTBOOST procedure is obtained. This loss function is more robust in the presence of outliers and noise events for which the \ADABOOST algorithm may degrade. \todo{shrinkage} In the analyses, both boosting algorithms have been tested but only negligible differences in performance have been found. This however also validates the obtained performance of \todo{???}

The discrimination power of a trained \gls{bdt} can be assessed using the \glshere{roc} curve. In Fig.~\ref{fig:technique-bdt-rocs} various \gls{roc} curves are presented. The \gls{auc} values denote the area under the \gls{roc} curve with respect to random guessing. Here, the trained \gls{bdt} with an \gls{auc} of $31\%$ outperforms the single most-discriminating observables; the pseudorapidity of the untagged light jet and the reconstructed top quark mass.
