...
The modeling of the \wjets and \ttbar backgrounds is studied in control regions before performing the measurement. In the 2j0t control region, two mismodeled observables are found, after applying the \bdtqcd selection to reject the multijet background, which can be attributed to the \wjets background. 

The transverse momentum of the W~boson shown in Fig.~\ref{fig:polarization-wjets-reweighting-wpt} displays a softer spectrum in data compared to simulation. This effect can be explained as an insufficient modeling of the hadronic recoil in such events. It is typically defined for \zjets and \wjets events as the momentum which balances the vector boson momentum in the transverse plane. The recoil consists of hard jets but has also a soft component stemming from contributions by the underlying event and pileup. A simple reweighting of simulated events in bins of the W~boson \pt is applied to correct for this deviation. In the statistical evaluation, the difference between applying and not applying this reweighting is considered as a dedicated systematic uncertainty.
...
\myfigure{\label{fig:polarization-sherpa-cosTheta}Distributions of the polarization angle in 2j0t control region using an alternative \wjets sample simulated with \SHERPA: (a)~before and (b)~after reweighting the flavor ratio to \MG.}{
