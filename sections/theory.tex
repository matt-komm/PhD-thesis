...
A common particle scattering process is characterized by the incoming and outgoing particles as well as the interactions between them. It can be calculated using perturbation theory. The transition amplitude from the initial state to the final state is given by the \gls{smatrix} defined as

\begin{align}
\mathcal{S}&=\lim_{t\to\infty}\langle\psi_{1}^\mathrm{out}\ldots\psi_\mathrm{N}^\mathrm{out}|e^{-iH(t)}|\psi_{1}^\mathrm{in}\ldots\psi_\mathrm{N}^\mathrm{in}\rangle \\
&=\sum_{n}^{\infty}\int \mathrm{d}x_{1}^{4}\ldots\mathrm{d}x_{n}^{4}~\langle\psi_{1}^\mathrm{out}\ldots\psi_\mathrm{N}^\mathrm{out}|T[\mathcal{H}_{1}\ldots\mathcal{H}_{n}]|\psi_{1}^\mathrm{in}\ldots\psi_\mathrm{N}^\mathrm{in}\rangle
\end{align}

given the interaction encoded in the Hamiltonian function $H$ or corresponding density $\mathcal{H}$.
...
matrix elements, time ordered operator=green function, cross sections (unitarity bound), decay rates, angles (w polarizations)/spin matrix, renormalization?, nlo calculations
