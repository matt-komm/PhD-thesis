...
In direct searches for \gls{bsm} signals one usually probes only for a certain model within its given parameter space. Typical models are the \gls{mssm}~\cite{Csaki:1996ks} which introduces a superpartner for each \gls{sm} particle using a minimal number of new parameters or the \gls{2hdm}~\cite{Branco:2011iw} where the Higgs sector is extended by an additional Higgs doublet. Since no signal has been found yet it may be that the energy scale at which new \gls{bsm} signal appear in direct searches is not reachable. This however motivates to probe for deviations of \gls{sm} parameters like the couplings which could receive corrections from potential new particles or interactions nonetheless. 

\myfigure{\label{fig:theory-t-channel-eff}Potential \gls{bsm} Feynman diagram of single-top-quark production via flavor-changing neutral current interaction. At low energies $p\ll m_\chi$ the propagator in (a) can be approximated as an anomalous 4-fermion coupling~(b).}{
\subfloat[]{\parbox{0.4\textwidth}{\centering\includegraphics[scale=0.75]{figures/theory/t-channel-chi.pdf}\\$\propto\frac{1}{p^2-m^{2}_{\chi}}$}}
\subfloat[]{\parbox{0.4\textwidth}{\centering\includegraphics[scale=0.75]{figures/theory/t-channel-contact.pdf}\\$\propto\frac{1}{m^{2}_{\chi}}$}}
}
