...
\myfigure{\label{fig:theory-t-channel-prod-decay}Sketch of spin orientations in $t$-channel single-top-quark production and decay inspired from Ref.~\cite{Schwienhorst:2010je}.}{
\includegraphics[scale=0.75]{figures/theory/t-channel-prod-decay.pdf}
}

A crucial ingredient still missing in this argumentation is the fact that the spectator quark has to be of down-type flavor. This is true in about $80\%$ for $t$-channel top quark production but only correct in $31\%$ of all cases for top antiquark production. Here, the down-type quark is found with a probability of $69\%$ in the initial state instead. However, the spectator quark is only mildly deflected when recoiling against the $\mathrm{W}$~boson. It will therefore be still close to the direction of the down-type quark momentum~\cite{Bernreuther:2008ju}.

Exemplary, the expect top quark polarizations in the \gls{sm} at \gls{lo} and \gls{nlo} are listed in Tab.~\ref{tab:theory-singletop-sm-polarization} at $7~\TeV$. The combined polarization has been calculated using a weighted sum with the corresponding cross sections of $\sigma_\mathrm{t}^\scriptn{7~\TeV}=41.8^{+1.8}_{-1.5}~\pb$ and $\sigma_{\bar{\mathrm{t}}}^\scriptn{7~\TeV}=22.0^{+1.3}_{-1.2}~\pb$ as weights\footnote{These cross sections are calculated with the same setup as used for Tab.~\ref{tab:theory-singletop-xsecs}}. At $8~\TeV$, a similar polarization of $|\mathrm{P}_{\mathrm{t}+\bar{\mathrm{t}}}^\mathrm{8~\TeV}|=0.88$~\cite{Khachatryan:2015dzz} is expected using the \gls{nlo} event generator \textsc{Powheg}.

\mytable{\label{tab:theory-singletop-sm-polarization}Expected polarizations of top quarks and top antiquarks in $t$-channel single-top-quark production at $7~\TeV$. Values are taken from Ref.~\cite{Schwienhorst:2010je}. Uncertainties are omitted.}{
\begin{tabular}{|c| r@{.}l r@{.}l r@{.}l|}
\hline
            & \multicolumn{2}{c}{$\mathrm{P}_\mathrm{t}$} & \multicolumn{2}{c}{$\mathrm{P}_{\bar{\mathrm{t}}}$} & \multicolumn{2}{c|}{$|\mathrm{P}_{\mathrm{t}+\bar{\mathrm{t}}}^\mathrm{7~\TeV}|$}     \\
\hline
\hline
\gls{lo}    & 0&99                             & -0&93                                   & 0&95          \\
\gls{nlo}   & 0&91                             & -0&86                                   & 0&88          \\
\hline
\end{tabular}
}

extended W boson helicities
