...
where the \glshere{me}, $\mathcal{M}$, captures the interaction terms at all orders of the perturbative series. \todo{check if the amplitude eq fits here} If $\mathcal{H}_\mathrm{int}>1$ the series does not converge and one says the problem is not solvable perturbatively. For instance, the ground states of hadrons can not be calculated perturbatively\footnote{An alternative approach based on lattice gauge theory allowed to solve this problem~\cite{Durr:2008zz}.} because of the running of the strong coupling constant.
...
\todo{add how much DM/energy is there}
