...
A common particle scattering process is characterized by the incoming and outgoing particles as well as the interactions between them. It can be calculated using perturbation theory. Here, the \gls{smatrix} allows to describe the transition between two multiparticle states which is defined in the Heisenberg picture as

\begin{align}
\mathcal{S}&=\mathrm{T}\exp\Big(-i\int\mathrm{d}^{4}x~\mathcal{H}_\mathrm{int}(t)\Big)\\
&=\sum_{n=0}^{\infty}\frac{(-i)^{n}}{n!}\int_{-\infty}^{\infty}\mathrm{d}^{4}x_{1}\ldots \int_{-\infty}^{\infty}\mathrm{d}^{4}x_{n}~\mathrm{T}\big(\mathcal{H}_\mathrm{int}(t_{1})\ldots\mathcal{H}_\mathrm{int}(t_{n})\big) \label{eq:theory-dyson}
\end{align}

given the interaction encoded in the Hamiltonian density $\mathcal{H}_\mathrm{int}=\mathcal{H}-\mathcal{H}_\mathrm{free}$. The operator $\mathrm{T}$ ensures that $\mathcal{H}_\mathrm{int}(t_{i})$ are ordered by time. The transition amplitude from an initial state to a final state with particles $\psi_i^\mathrm{in/out}$ respectively is given by
...
&=\langle\psi_{1}^\mathrm{out}\ldots\psi_{N^{\prime}}^\mathrm{out}\,|\,\psi_{1}^\mathrm{in}\ldots\psi_{N}^\mathrm{in}\rangle+i(2\pi)^{4}\delta^{4}\big(\Sigma p^\mathrm{in}_{i}-\Sigma p^\mathrm{out}_{j}\big)\mathcal{M}
\end{align}

where $\mathcal{M}$ captures the interaction terms at all orders of the perturbative series.

Given a flux $L=\rho v$ of incoming particles with velocity $v$ which will interact, the cross section $\sigma$ is defined as the number of interactions per unit density~($\rho=1$). It is usually denoted in the unit ``barn''~[$\mathrm{b}$] which is defined as $1~\mathrm{b}\equiv 10^{-28}~\mathrm{m}^{2}$.  The number of events per time is given by $\mathrm{d}N/\mathrm{d}t=L\sigma$. The flux $L$ is commonly referred to as luminosity.
...
where $\mathrm{d}\Phi^\mathrm{out}$ integrates over the phase space of the scattered outgoing particle momenta. 

The optical theorem follows from the unitarity $1=\mathcal{S}^{\dagger}\mathcal{S}$ of the \gls{smatrix}. It states that the imaginary part of a scattering amplitude $\mathcal{M}(i\to f)$ is directly related to the scattering amplitude of producing all possible intermediate particles $X$ from the initial times the final state $\mathcal{M}^\dagger(i\to X)\mathcal{M}(f\to X)$. This leads to general constraints on the total cross section of a process. For example, the scattering amplitude of two particles can be expressed via the optical theorem as
...
A theory may describe experimental data well but still violates the unitarity bound at higher energies beyond the experimental probing capabilities. This suggests a new physics model which restores unitarity through new particles or interactions at higher energies. This is commonly referred to as \gls{uv} completion. A unitarity-violating theory can therefore just reflect the low energy limit of an unknown \gls{uv}-complete theory.


 decay rates, angles (w polarizations)/spin matrix, renormalization?, nlo calculations
