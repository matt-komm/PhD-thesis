...
The procedure of symmetry breaking through a non-zero \gls{vev} allows to introduce mass terms for the \wboson and \zboson bosons starting from a local gauge invariant theory. Its value can be determined from the Fermi coupling, measured in muon decays, as $v = (\sqrt{2}\mathrm{G_{F}})^{-1/2}\approx 246~\GeV$~\cite{PhysRevLett.106.041803}. Additionally, the Higgs boson is predicted which was discovered in July 2012 independently by the ATLAS~\cite{Aad:2012tfa} and CMS~\cite{Chatrchyan:2012xdj} collaborations and thus completing the electroweak theory.
...
where a scattering can be initiated by a quark/antiquark pair or two gluons in the initial state.
...
However, a theory may describe experimental data well but still violates the unitarity bound at higher energies. Such a case would hint towards a new physics model which restores unitarity by introducing new particles or interactions at higher energies. This is commonly referred to as \gls{uv} completion. Hence, a unitarity-violating theory may just reflect the low energy limit of an unknown \gls{uv}-complete theory beyond the experimental probing capabilities. 
...
The top quark is the heaviest known fundamental particle so far. Its very high mass of $m_\mathrm{t}=173.34\pm0.71~\GeV$~\cite{ATLAS:2014wva} -- comparable to the mass of a gold atom~($\approx 184~\GeV$) -- is close to the minimum of the Higgs potential which implies a Yukawa coupling of $|\lambda_\mathrm{t}|\approx 1$. Therefore, it may offer the means to understand the mechanisms of electroweak symmetry breaking itself. Furthermore, in various extensions to the \gls{sm}, new massive particles are predicted which can interact with top quarks. For example, many models~(see Ref.~\cite{Boos:2006xe} and references therein) predict additional $\wboson^{\prime}$ and $\zboson^{\prime}$ bosons which are heavier versions of their \gls{sm} counterparts. Hence, the production mechanisms of top quarks, their strength and coupling structure, may be altered through direct decays of such particles or higher order corrections.

In 1995, the top quark was discovered by the \gls{cdf}~\cite{Abe:1995hr} and \gls{d0}~\cite{Abachi:1994td} collaborations at the Tevatron collider, Fermilab using $p\bar{p}$ collision data. The production of top quark / antiquark pairs which occurs through strong interactions was analyzed. Already before its direct observation, the discovery of the bottom quark~\cite{Augustin:1975yq,PhysRevLett.39.252} and the attempt to explain CP-violation by introducing the \gls{ckm} matrix hinted strongly towards its existence~\cite{Kobayashi01021973}.

Another milestone was the observation of single top quark production which occurs through electroweak interactions only. This more rarer process was discovered in 2009 also at the Tevatron collider~\cite{PhysRevLett.103.092002,PhysRevLett.103.092001}. Only very few candidate events ... 

need LHC to probe top quark inside out!!!


\subsection{Production processes}

ttbar, t,tW,s channel, associated production (e.g. tHq), decay + coupling structure, Vtb
