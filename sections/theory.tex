...
where however a four-fermion contact interaction term remains~\cite{Bach:2012fb}. This interaction cannot contribute to top quark production and decays via a $\mathrm{Wtb}$~vertex directly. However, the four-fermion vertex should not be neglected when studying  single top quark production in particular. It can contribute a $\mathrm{udbt}$~vertex (similar to Fig.~\ref{fig:theory-t-channel-eff-ir}) as an addition to the $\mathrm{Wud}\text{+}\mathrm{Wtb}$~vertices which e.g. occur in the production via $t$~channel.

Various (pseudo) observables are proposed in literature to derive limits on these anomalous couplings from experimental data. A few of them are the mentioned top quark polarization, the $\mathrm{W}$~boson helicity fractions, the \ttbar spin correlation, and the single-top-quark cross sections in the three channels. In this thesis, a first measurement of the top quark polarization has been performed to be used in future fits.
