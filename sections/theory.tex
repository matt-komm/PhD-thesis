...
\subfloat[\label{fig:theory-t-channel-chi-uv}$t$~channel]{\parbox{0.35\textwidth}{\centering\includegraphics[scale=0.75]{figures/theory/t-channel-chi.pdf}\\$\propto\lambda^2\frac{i}{p^2-m^{2}_{\chi}}$}}
\subfloat[\label{fig:theory-t-channel-eff-ir}effective interaction]{\parbox{0.35\textwidth}{\centering\includegraphics[scale=0.75]{figures/theory/t-channel-contact.pdf}\\$\propto-\lambda^2\frac{1}{m^{2}_{\chi}}$}}
}

\gls{eft} can be used to characterize such deviations from the coupling structure in a model-independent manner. It can be derived using an \gls{ope} as first proposed in Ref.~\cite{Wilson:1972ee}. Here the product of operators is expanded as

\begin{equation}
O_{1}(x_{1})\ldots O_{n}(x_{n})=\sum_{i}c_{1\ldots n}^{i}(x_{1},\ldots x_{n})O_{i}^\mathrm{eff}(x_{n})
\end{equation}

where $O_{i}^\mathrm{eff}$ is now an effective operator and $c^{i}$ are 

This is applicable only if the low energy limit of a \gls{uv}-complete theory is relevant. Otherwise unitarity as discussed in Sec.~\ref{sec:theory-observables} will be violate if the energy scale of a process approaches the scale of the concrete \gls{bsm} theory which would be $m_\chi$ in the example above. At such energy scales the \gls{eft} approach is invalid.
