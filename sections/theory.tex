...
\todo{find out if this is for fixed initial/final state momenta}
...
\todo{ref to https://arxiv.org/abs/1404.1585?}
...
\todo{the operators are renormalizable through ope for low scales?}
...
\begin{equation}
\mathcal{L}_\mathrm{eff}=\mathcal{L}_\mathrm{\gls{sm}}^\mathrm{(4)}+\frac{1}{\Lambda}\sum_{i}c_{i}^\mathrm{(5)}{O}_{i}^\mathrm{(5)}+\frac{1}{\Lambda^{2}}\sum_{i}c_{i}^\mathrm{(6)}{O}_{i}^\mathrm{(6)}+\mathcal{O}\Big(\frac{O^\mathrm{(7)}}{\Lambda^3}\Big)
\end{equation}
...
For the top quark sector only a subset of operators are relevant. In particular, the following operators contribute to the $\mathrm{Wtb}$~vertex

\begin{align}
\mathcal{L}_\mathrm{eff.}^\mathrm{Wtb}=\mathcal{L}_\mathrm{SM}^\mathrm{Wtb}&+\frac{i}{\Lambda^2}c_{\phi\mathrm{q}}^{33}\Big(\phi^\dagger\omega_{a}\mathrm{D}_\mu\phi\Big)\Big(\bar{\mathrm{Q}}_\mathrm{L}^{3}\gamma^{\mu}\omega^{a}\mathrm{Q}_\mathrm{L}^{3}\Big)+\frac{i}{\Lambda^2}c_{\phi\phi}^{33}\Big(\tilde{\phi}^\dagger\mathrm{D}_\mu\phi\Big)\Big(\bar{\mathrm{u}}_\mathrm{R}^{3}\gamma^{\mu}\mathrm{d}_\mathrm{R}^{3}\Big) \nonumber\\
...
\begin{align}
\mathcal{L}_\mathrm{eff.}^\mathrm{Wtb}=&-\frac{g}{\sqrt{2}}\bar{\mathrm{b}}\gamma^{\mu}\big(\mathrm{V}_\mathrm{L}\mathrm{P}_\mathrm{L}+\mathrm{V}_\mathrm{R}\mathrm{P}_\mathrm{R}\big)\mathrm{t}\mathrm{W}_\mu^{\rmminus}\\
...
where $\mathrm{V}_\mathrm{L,R}$ and $\mathrm{g}_\mathrm{L,R}$ denote the vector- and tensor-like anomalous couplings respectively~\cite{AguilarSaavedra:2008zc}. However, the operator $O_\mathrm{qW}^{ij}$ is missing in Eq.~\ref{eq:theory-eft-lagrangian-operators}. Its contribution can be mostly absorbed by the other operators as
...
&\supset\big(\text{terms }\propto\mathcal{L}_\mathrm{Wtb}\big)~+~\frac{g\,\mathrm{Re}(c_\mathrm{qW})}{\Lambda^2}\big(\bar{\mathrm{b}}\gamma^{\mu}\mathrm{P}_\mathrm{L}\mathrm{t}\big)\big(\bar{\mathrm{q}}\gamma_\mu\mathrm{P}_\mathrm{L}\mathrm{q}^\prime\big)+\mathrm{\gls{hc}}
\end{align}

where however a 4-fermion contact interaction term remains~\cite{Bach:2012fb}. This interaction cannot contribute to top quark production and decays via a $\mathrm{Wtb}$~vertex directly. However, the 4-fermion vertex should not be neglected because it can contribute a $\mathrm{udbt}$~vertex as an addition to the $\mathrm{Wud}\text{+}\mathrm{Wtb}$~vertices which e.g. occur in the production of single top quarks via $t$~channel.
