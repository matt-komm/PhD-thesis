...
The cornerstone of such a calculation is the so-called \gls{smatrix} which describes the transition of a system from an initial to a final multiparticle state. In the Heisenberg picture it is defined as

\begin{align}
\mathcal{S}&=\mathrm{T}\exp\Big(-i\int\mathrm{d}^{4}x~\mathcal{H}_\mathrm{int}(t)\Big)\\
&=\sum_{n=0}^{\infty}\frac{(-i)^{n}}{n!}\int_{-\infty}^{\infty}\mathrm{d}^{4}x_{1}\ldots \int_{-\infty}^{\infty}\mathrm{d}^{4}x_{n}~\mathrm{T}\big(\mathcal{H}_\mathrm{int}(t_{1})\ldots\mathcal{H}_\mathrm{int}(t_{n})\big) \label{eq:theory-evolution-series}
\end{align}

where the interaction is encoded in the non-free part of the Hamiltonian density $\mathcal{H}_\mathrm{int}=\mathcal{H}-\mathcal{H}_\mathrm{free}$. The operator $\mathrm{T}$ ensures that products of $\mathcal{H}_\mathrm{int}(t_{i})$ are ordered by time. The transition amplitude from an initial state to a final state with particles $\psi_i^\mathrm{in/out}$ respectively can hence be calculated using
...
where $\mathrm{d}\Phi^\mathrm{out}$ integrates over the phase space of the scattered outgoing particle momenta. For a theory with coupling constant $\alpha$, one can expand the cross section in terms of the perturbative series~(Eq.~\ref{eq:theory-evolution-series}) as

\begin{equation}
\sigma=\underbrace{\underbrace{\underbrace{\alpha\cdot\sigma_\mathrm{1}}_\mathrm{LO}+\alpha^{2}\cdot\sigma_{2}}_\mathrm{NLO}+\alpha^{3}\cdot\sigma_{3}}_\mathrm{NNLO}+\ldots
\end{equation}
...
However, a theory may describe experimental data well but still violates the unitarity bound at higher energies. This would hint towards a new physics model which restores unitarity through new particles or interactions at higher energies. This is commonly referred to as \gls{uv} completion. Hence, a unitarity-violating theory may just reflect the low energy limit of an unknown \gls{uv}-complete theory beyond the experimental probing capabilities. 
