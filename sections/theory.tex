...
\begin{align}
\vec{\mathrm{E}}_\mathrm{L}&=\Bigg\{\colvec{2}{e^{\rmminus}_\mathrm{L}}{\nu_\mathrm{e,L}},\colvec{2}{\mu^{\rmminus}_\mathrm{L}}{\nu_{\mu,\mathrm{L}}},\colvec{2}{\tau^{\rmminus}_\mathrm{L}}{\nu_{\tau,\mathrm{L}}}\Bigg\} \label{eq:theory-su2-leptons} \\
...
The top quark decays via electroweak interactions almost exclusively into on-shell W bosons and a b quark because of its high mass. This leads to a very short lifetime of only $1/\Gamma_\mathrm{t}\approx 5\cdot10^{-25}~\mathrm{s}$~\cite{Olive:2016xmw} which is even below the typical hadronization timescale of $1/\Lambda_\mathrm{QCD}\approx 10^{-23}~\mathrm{s}$. Hence, the top quark spin orientation is not lost due to soft gluon emissions. The spin orientation can even be reconstructed from the angular distributions of its decay products since electroweak interactions via \wboson exchange features a \gls{va} coupling structure. This offers the possibility to study the polarization of top quarks as a pseudo observable in various \gls{sm} processes. One can utilize top quark decays to investigate the polarization states of the \wboson boson~\cite{AguilarSaavedra:2010nx} in $\mathrm{t}\to\mathrm{b}\ell\bar{\nu}$ decays. For this, the top quark decay width is decomposed into
...
where $\mathrm{F}_{\rmplus}$, $\mathrm{F}_{\rmminus}$, and $\mathrm{F}_{0}$ denote the left-, right-, and longitudinal polarization fraction. These dimensionless form factors are normalized as $\mathrm{F}_{\rmplus}+\mathrm{F}_{\rmminus}+\mathrm{F}_{0}=1$. The angle $\cos\theta^\star(\vec{\mathrm{s}}_\mathrm{t},\vec{\ell})$ 
