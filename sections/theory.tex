...
where $\mathrm{d}\Phi^\mathrm{out}$ integrates over the phase space of the scattered outgoing particle momenta. In case of e.g. hadron--hadron collisions, Eq.~\ref{eq:theory-xsec-calculation} only calculates the partonic cross section. An additional integration over the \glspl{pdf} is required to sum up the individual contributions per parton flavor and per momentum fraction. Exemplary, for the process $\mathrm{pp}\to \bar{\mathrm{t}}\mathrm{t}$ this leads to

\begin{equation}
\sigma(\mathrm{pp}\to \bar{\mathrm{t}}\mathrm{t})=\sum_{i,j}\iint\mathrm{d}x_{1}\,\mathrm{d}x_{2}~\mathrm{PDF}(x_{1},f_{i},\mu_\mathrm{F})\,\mathrm{PDF}(x_{2},f_{j},\mu_\mathrm{F})\,\sigma(f_{i}f_{j}\to X)
\end{equation}
...
Depending at which term the series is cut off one speaks of \gls{lo}, \gls{nlo}, or \gls{nnlo} accuracy in $\alpha$. In general, observables calculated beyond \gls{lo} tend to be less affected by theoretical uncertainties which originate from a variation of the renormalization and factorization scales\footnote{\gls{lo} cross sections can only be monotonous functions of $\mu_\mathrm{R}$ since $\sigma\propto\alpha(\mu_\mathrm{R})$. Hence, an uncertainty due to a scale variation can be even considered meaningless.}.
...
The top quark is the heaviest known fundamental particle so far. Its mass of $m_\mathrm{t}=173.34\pm0.71~\GeV$ is close to the minimum of the Higgs potential which implies a Yukawa coupling of $|\lambda_\mathrm{t}|\approx 1$. It is therefore considered that it may play an important role in the electroweak symmetry breaking mechanism itself. Furthermore, new physics ...

The 1995, it was discovered by the \gls{cdf}~\cite{Abe:1995hr} and \gls{d0}~\cite{Abachi:1994td} collaborations at the Tevatron collider, Fermilab using $p\bar{p}$ collision data.

introduction, discovery of ttbar, ST, decay

\subsection{Production processes}

ttbar, t,tW,s channel, associated production (e.g. tHq)
