...
\begin{tabular}{|c|c c|}
\hline
decay product $X$       & \hspace{0.5cm}$\alpha_{X}^\mathrm{LO}$\hspace{0.5cm}  & \hspace{0.5cm}$\alpha_{X}^\mathrm{NLO}$\hspace{0.5cm} \\
\hline
\hline
...
The theoretical cross sections of the three single top quark production modes for \gls{cm} energies of $8$ and $13~\TeV$, relevant for this thesis, are listed in Tab.~\ref{tab:theory-singletop-xsecs}. These have been calculated at \gls{nlo} in \gls{qcd} with \textsc{\gls{hathor}}\,$2.1$~\cite{Aliev:2010zk,Kant:2014oha} using a top quark mass of $m_\mathrm{t}=172.5~\GeV$ and factorization and renormalization scales of $\mu_\mathrm{R}=\mu_\mathrm{F}=m_\mathrm{t}$. The \gls{pdf} uncertainty includes also the uncertainty on \as.

\mytable{\label{tab:theory-singletop-xsecs}Single top quark cross sections per production mode in $\pb$.}{
\begin{tabular}{|c|c c|}
\hline
mode            & $8~\TeV$          & $13~\TeV$     \\
\hline
\hline
$t$-channel     & $84.7^{+2.6}_{-1.7}\,\mathrm{(scale)}\pm2.8\,\mathrm{(PDF)}$ & $217.0^{+6.6}_{-4.6}\,\mathrm{(scale)}\pm6.2\,\mathrm{(PDF)}$ \\
tW-channel      & & \\
$s$-channel     & & \\
\hline
\end{tabular}
