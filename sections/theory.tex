...
which denotes whether the spin $\vec{s}$ is aligned along~($H=1$) or against~($H=-1$) the momentum axis. For massive particles, Eq.~\ref{eq:theory-diracL} does not decouple since chirality is not Lorentz invariant. \todo{add that CPT is considered invariant}
...
One says that this non-zero \gls{vev} ``breaks'' the $\mathrm{SU(2)}$ symmetry. In fact, the symmetry still exists but is ``hidden'' when viewed from $x$ where $\phi(x)=\phi_0$ instead of $x=0$. When parameterizing $\phi$ around the minimum as
...
\begin{equation}
\mathcal{L}_{\wboson~\mathrm{interaction}}=\frac{g}{\sqrt{2}} \bar{\mathrm{u}}^{\prime i}_\mathrm{L}\gamma^{\mu}\underbrace{(U^\dagger_\mathrm{u}U_\mathrm{d})_{ij}}_{=\mathrm{V}_{ij}}\mathrm{d}^{\prime j}_\mathrm{R} W^{+}_\mu+\mathrm{\gls{hc}}
\end{equation}
...
Given a flux $L=\rho v$ of incoming particles with velocity $v$ which will interact, the cross section $\sigma$ is defined as the number of interactions per unit density~($\rho=1$). It is usually denoted in the unit ``barn''~[$\mathrm{b}$] which is defined as $1~\mathrm{b}\equiv 10^{-28}~\mathrm{m}^{2}$.  The number of interaction events per time is given by $\mathrm{d}N/\mathrm{d}t=L\sigma$. The flux $L$ is commonly referred to as luminosity.
...
One can expand a cross section in terms of the coupling constant $\alpha$ using the perturbative series~(Eq.~\ref{eq:theory-dyson-series}) as
...
With start of the physics program at the \gls{lhc} in 2010, the production mechanisms of top quark can be precisely probed in proton-proton collisions now. This thesis focuses on the production of single top quark events at center-of-mass energies of $8~\TeV$ and $13~\TeV$.

%##############################################
\subsection{Decay and production processes}
%##############################################

The top quark decays almost exclusively via electroweak interactions into on-shell W bosons and a b quark because of its high mass. This leads to a very short lifetime of $1/\Gamma_\mathrm{t}\approx 5\cdot10^{-25}~\mathrm{s}$~\cite{Olive:2016xmw} which is below the typical hadronization timescale of $1/\Lambda_\mathrm{QCD}\approx 10^{-23}~\mathrm{s}$. Hence, the top quark spin orientation is not lost due to soft gluon emission. The orientation can be even reconstructed from the angular distributions of its decay products since electroweak interactions via \wboson exchange feature a \gls{va} coupling structure. This is offers the possibility to study the polarization of top quarks in various \gls{sm} processes and beyond. One can already use top quark decays to investigate the polarization states of the \wboson boson~\cite{AguilarSaavedra:2010nx}. The top quark decay width can be decomposed into

\begin{equation}
\frac{\mathrm{d}\Gamma}{\Gamma\cdot\mathrm{d}\cos\theta^\star}=\tfrac{3}{8}\big(1+\cos\theta^\star\big)^{2}\mathrm{F}_{\rmplus}+\tfrac{3}{8}\big(1-\cos\theta^\star\big)^{2}\mathrm{F}_{\rmminus}+\tfrac{3}{4}\sin^{2}\theta^\star\mathrm{F}_{0}
\end{equation}

where $\mathrm{F}_{\rmplus}$
