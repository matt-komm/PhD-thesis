...
\begin{align}
\mathcal{S}&=\mathrm{T}\big(e^{-i\int\mathrm{d}^{4}x~\mathcal{H}_\mathrm{int}(t)}\big)\\
...
\end{align}
c
where $\mathcal{M}$ captures all interaction terms at any order.

Given a flux $L=\rho v$ of incoming particles with velocity $v$, the cross section $\sigma$ denotes the number of interactions per unit density~($\rho=1$) such that $\mathrm{d}N/\mathrm{d}t=L\sigma$ denotes the number of events per time. The flux $L$ is commonly called luminosity and measured as a multiple of inverse ``barn''~[$\mathrm{b}$] where $1~\mathrm{b}\equiv 10^{-28}~\mathrm{m}^{2}$. 

For a collider experiment with beam energies $E_{i}$ one finds
...
\end{align}
 ...
 (unitarity bound), decay rates, angles (w polarizations)/spin matrix, renormalization?, nlo calculations
