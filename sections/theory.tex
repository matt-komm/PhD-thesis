...
The top quark decays via electroweak interactions almost exclusively into on-shell W bosons and a b quark because of its high mass. This leads to a very short lifetime of only $1/\Gamma_\mathrm{t}\approx 5\cdot10^{-25}~\mathrm{s}$~\cite{Olive:2016xmw} which is even below the typical hadronization timescale of $1/\Lambda_\mathrm{QCD}\approx 10^{-23}~\mathrm{s}$. Hence, the top quark spin orientation is not altered by non-perturbative soft gluon emissions. The spin orientation can even be reconstructed from the angular distributions of its decay products since electroweak interactions via $\mathrm{W}$ boson exchange features a \gls{va} coupling structure. This offers the possibility to study the polarization of top quarks as a pseudo observable in various \gls{sm} processes. One can utilize the top quark decay chain $\mathrm{t}\to\mathrm{b}\ell\bar{\nu}$ to investigate the polarization states of the \wboson boson~\cite{AguilarSaavedra:2010nx}. For this, the top quark decay width is decomposed into
...
where $\mathrm{F}_\mathrm{L}$, $\mathrm{F}_\mathrm{R}$ denote the left-, right-handed, and longitudinal polarization fraction respectively. Theses dimensionless form factors are normalized as $\mathrm{F}_\mathrm{L}+\mathrm{F}_\mathrm{R}+\mathrm{F}_{0}=1$. The angle $\theta^\star(\vec{s}_\mathrm{W},\vec{p}_{\ell})$ is taken between the lepton momentum $\vec{p}_{\ell}$ in the $\mathrm{W}$ boson rest frame and a spin quantization axis $\vec{s}_\mathrm{W}$. A common basis is the helicity basis where the negative top quark momentum $\vec{s}_\mathrm{W}=-\vec{p}_\mathrm{t}^\mathrm{(W)}$ is used to quantize the $\mathrm{W}$ boson spin\footnote{There are multiple equivalent definitions in literature: $\vec{s}_\mathrm{W}=-\vec{p}_\mathrm{t}^\mathrm{(W)}=\vec{p}_\mathrm{W}^\mathrm{(t)}=\vec{p}_\mathrm{b}^\mathrm{(W)}$}. Figure~\ref{fig:theory-top-decay}

\myfigure{\label{fig:theory-top-decay} Decay of the top quark through \wboson exchange: (a)~Feynman diagram; (b)~helicity angle in $\mathrm{W}$ boson rest frame.}{
\subfloat[]{\includegraphics[scale=0.75]{figures/theory/t_decay.pdf}}\hspace{0.15\textwidth}
\subfloat[]{\includegraphics[scale=0.75]{figures/theory/whel.pdf}}
}
