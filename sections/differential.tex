...
The final schemes chosen for the measurement of the differential cross section as a function of the top quark \pt and rapidity are presented in Tabs.~\ref{tab:diff13-ps-top-pt} and~\ref{tab:diff13-ps-top-y} together with the calculated stabilities and purities per parton-level bin. Overall, both quantities are found to be above 50\% with the exception of the purity in the last rapidity bin which amounts to only 41\%.

\mytable{\label{tab:diff13-ps-top-pt}Stabilities and purities per bin of the top quark \pt distribution at parton level.}{
...
\mytable{\label{tab:diff13-ps-top-y}Stabilities and purities per bin of the top quark rapidity distribution at parton level.}{
\begin{tabular}{@{}l c c c c c@{}}
...
The measurement is affected by various sources of systematic uncertainties. For each source new templates are derived which reflect a systematic variation by one standard deviation. Those are then subjected to the ML-fits, the \gls{bdt}, and the unfolding. Special care is taken in the unfolding step since not only the templates can change under a systematic variation but also the response matrices themselves. The resulting differences in yield with respect to the differential cross section using the nominal templates are quoted as individual uncertainties. 

To calculated the 

\begin{equation}
y_{i}^\mathrm{total}=y_{i}^\mathrm{nom.}+\sum_{j}^\mathrm{syst.}\,\mathsf{N}\Big(0,\,\Delta\big(y_{i}^\mathrm{nom.},\,y_{i}^{\mathrm{\pm syst.\,}j}\big)\Big)\,,
\end{equation}

\begin{description}
\item[Background composition] \zjets tW varied by $\pm20\%$ keeping 
\item[Multijet template shape] Isolation region
\item[Experimental uncertainties] btagging, jes,jer,pileup,trigger,id,iso
\item[Generator and hadronization model] \MGAMC to \POWHEG; \PYTHIA to \HERWIG
\item[top quark mass] $172.5\pm1~\GeV$
\item[\acrlong{pdf}] 102 variation of NNPDF3.0 set~\cite{}
\item[Renormalization and factorization scales]
\item[\ttbar \pt reweighting]
\end{description}
