...
In the following, the components of \gls{cms} are briefly described and their purpose is motivated. More information can be found in Ref.~\cite{Chatrchyan:2008aa}.
...
Pixel modules are located next to the beam pipe where the particle flux is particularly high. They are installed in three barrel layers~(\gls{bpx}) at radii of 4.4, 7.3, and 10.3~cm, and in two endcap disks~(\gls{fpx}) per side. A pixel module has a cell size of $100\times150~\upmu\mathrm{m}^{2}$ which allows local 2D hit position measurements. Local positions can be transformed into global 3D positions by accounting for the surface orientation of each module. The channel occupancy of the pixel subdetector which is defined as the fraction of readout channels yielding a signal has been measured in data to range between 0.002\range0.02\% only~\cite{Chatrchyan:2014fea}. This facilitates the overall search for particles trajectories by generating seeds from precise hits on the pixel modules.

In the other parts of the inner tracking system silicon strip modules are installed. The strip modules allow to measure a local 1D hit position perpendicular to the strip direction. However, a few layers and disks contain glued double-sided strip modules with an angle of 0.1~rad which allow to reconstruct matched 2D hits by combining 1D hits from both sides as indicated in Fig.~\ref{fig:experiment-tracker}. The modules are organized in four inner barrel layers~(\gls{tib}) at radii of 20\range50~cm; six outer barrel layers~(\gls{tob}) at radii of 55\range116~cm; three inner endcap disks~(\gls{tid}); nine outer endcap disks~(\gls{tec}). In the barrel the strip directions on each module are aligned along the z-axis with a strip-to-strip distance that varies between $80\range183~\upmu\mathrm{m}$. In the endcap disks, the modules are wedge-shaped with their strips running along the radial axis whose distance ranges between $100\range184~\upmu\mathrm{m}$.  ???.  

The pixel and strip sensors are readout by 


cooling, channel readout electronics, de/dx
