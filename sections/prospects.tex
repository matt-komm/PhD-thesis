...
\myfigure{\label{fig:prospects-bdt-tch} Distributions of the \bdttch discriminant, trained for separating signal from \ttbar and \wjets events, in 2j1t (a)~electron and (b)~muon channel.}{
...
An additional \gls{bdt} has been proposed to separate between the two main backgrounds, \wjets and \ttbar, in the 2j1t signal region. Since it is evaluated in a signal-depleted region only, observables correlated to the unfolding variables can be safely used in its training without introducing a potential bias on signal events. The resulting discriminant with an \gls{auc} of about 25\% provides additional sensitivity to the W+heavy-quark-flavor yield in the \gls{ml} fit which can otherwise not be constrained from control regions.
