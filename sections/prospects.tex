...
\intro{Improvements for future measurements of differential cross sections are investigated. These are showcased for an envisaged measurement of differential cross sections as a function of the top quark transverse momentum, rapidity, and polarization angle using $\mathit{36~fb^{-1}}$ of \gls{pp} collision data recorded with the \gls{cms} experiment in 2016. First, the training of a new \gls{bdt} discriminant for separating W+jets from $\mathit{t\bar{t}}$ events in the signal region is motivated which yields an enhanced sensitivity to the heavy-quark-flavor enriched W+jet background for which a control region can not be defined easily otherwise. Secondly, the requirements for measuring differential cross sections at particle level in addition to the parton level are discussed. This includes a technical study about finding an appropriate selection of event at particle level which has also been published in Ref.~\cite{particleStudies}. The chapter is concluded by presenting projections obtained with pseudo-data.}
...
\mytable{\label{tab:prospects-theo-xsecs}Theoretical \gls{sm} cross sections at 13~TeV used to normalize the exclusive \wjets samples. The cross section have been calculated at \gls{nlo} with the \MGAMC generator.}{
\begin{tabular}{@{}l  r@{}}
\toprule
Sample\hspace{3cm} & Cross section \\
\midrule
$\mathrm{W}\to\ell\nu\mathrm{\,\mbox{+}\,0~jets}$ & $49\,670~\pb$ \\
$\mathrm{W}\to\ell\nu\mathrm{\,\mbox{+}\,1~jet}$ & $8\,264~\pb$ \\
$\mathrm{W}\to\ell\nu\mathrm{\,\mbox{+}\,2~jets}$ & $2\,544~\pb$ \\
\bottomrule
\end{tabular}
}
...
Distributions of the reconstructed top quark mass in the 2j1t signal and in the 2j0t \wjets and 3j1t \ttbar control regions are presented in Fig.~\ref{fig:pospects-topmass} for the muon and electron channel. A fair modeling is observed for both channels although a slight slope can be seen in the data-to-\gls{mc} ratio for the electron channel in 3j1t which may however not be significant when evaluating the impact by systematic uncertainties. In the signal region the b-tag requirement reduces the contribution by multijet events in both channels to a comparable fraction as opposed to the 2j0t control region where the multijet fraction in the electron channel appears to be more than twice as in the muon channel.
...
\myfigure{\label{fig:prospects-bdt-tch} Distributions of the \bdttch discriminant, trained for separating signal from \ttbar and \wjets events, in 2j1t (a)~electron and (b)~muon channel. Data have been blinded in the signal-enriched region $\bdttch>0$.}{
\subfloat[]{\adjincludegraphics[height=4.8cm,trim={0 0 {0.16\width} 0},clip]{figures/prospects/plots/2j1t/electron_2j1t_bdt_tch_qcdnone_nol.pdf}}
\subfloat[]{\adjincludegraphics[height=4.8cm,trim={0 0 {0.\width} 0},clip]{figures/prospects/plots/2j1t/muon_2j1t_bdt_tch_qcdnone.pdf}}
}
