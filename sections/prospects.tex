...
\mytable{\label{tab:prospects-theo-xsecs}Theoretical \gls{sm} cross sections used to normalize the exclusive \wjets samples. The cross section have been calculated at \gls{nlo} with the \MGAMC generator.}{
\begin{tabular}{@{}l  r@{}}
\toprule
...
\myfigure[p]{\label{fig:pospects-topmass} Distributions of the reconstructed top quark mass for (left column)~electron and (right column)~muon channel: (top row)~signal region; (middle and bottom row)~\ttbar control regions.}{
