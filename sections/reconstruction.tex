...
where $\sigma_\mathrm{fid.}=\sigma\cdot A$ is the fiducial cross section observed within the luminometer acceptance $A$. Van der Meer~(\gls{vdm}) scans are performed during which the proton beams are first separated and then gradually crossed with each other. The fiducial cross section is estimated from such scans by fitting the beam profiles as a function of their separation providing an absolute calibration of the luminometers. The most precise luminosity estimations for the analyzed data in this thesis are obtained with the pixel cluster counting method~\cite{CMS-PAS-LUM-13-001} \todo{cite LUM-17-001 when approved}. Here, the rate is defined as the number of pixel clusters reconstructed in the second and third pixel barrel layers. The pixel cluster counting method profits from the low channel occupancy of the pixel detector which results in a very linear dependency between the cluster rate and the instantaneous luminosity even in high pileup collisions.
