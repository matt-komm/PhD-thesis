...
The vertex reconstruction tries to locate points of pp interaction from which sets of tracks originated. Clusters of tracks, which are close to the beam spot, are constructed using a deterministic annealing algorithm~\cite{726788}. Here, the association of a track to a vertex candidate is floating and modeled by a probability. A model similar to statistical mechanics is created where each track reflects a microstate. The free energy of the system is monitored while decreasing its temperature. If a minimum of the free energy is reached new vertices are added. 

The final vertex positions are estimated through a fit using the clustered tracks.
