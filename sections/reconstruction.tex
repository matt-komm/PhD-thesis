...
Its core consists of a linking algorithm which, given a particle hypothesis, links several subdetector information together. The linked elements can be charged-particle tracks from the inner tracker, calorimeter clusters from the \gls{ecal} or \gls{hcal}, and muon tracks. The algorithm starts by extrapolating charged-particle tracks into the calorimeter systems. In the \gls{ecal} the trajectory is extend to the expected depth of the shower maximum of an electron candidate. In the \gls{hcal} the trajectory is extrapolated up to one interaction length. A link is created if the trajectory is located inside the boundaries of a compatible \gls{ecal} or \gls{hcal} cell cluster within uncertainties. Photon candidates which could have been emitted tangentially from electron tracks are also created by extrapolating straight tracks from the electron track hits on tracker layers to compatible \gls{ecal} cells. For neutral particle candidates, links between calorimeter clusters are formed instead. Here, the algorithm starts first with the higher resolution clusters on the \gls{ecal} or \gls{ps} and extrapolates them to potential \gls{hcal} or \gls{ecal} clusters respectively which have a coarser granularity. The described reconstruction muon and electron tracks in Secs.~\ref{sec:reconstruction-muontracks} and~\ref{sec:reconstruction-electrontracks} is also considered part of the \gls{pf} algorithm.

Finally, the identification of particles is performed by utilizing the linked blocks. First, global \gls{pf} muons are easily identified through the unambiguous signal in the muon system. Then \gls{pf} electrons are identified through the \gls{gsf} tracking together with potential photons from bremsstrahlung. %For the remaining \gls{pf} candidates, their track quality is tightened to reject fake tracks which would spoil the energy resolution of the calorimeters.

rest pf with tracks are charged hadrons, others are neutral hadrons or photons

\myfigure{\label{fig:reconstruction-pfjets}Comparison of calorimeter jets and particle flow jets: (a)~spatial resolution in $\eta$; (b)~energy resolution. The figures are taken from Ref.~\cite{CMS:2009nxa}.}{
