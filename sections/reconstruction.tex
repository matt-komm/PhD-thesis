...
By propagating the corrected energy for \gls{pf} candidates clustered into jets the \gls{met} scale is improved. The correction can be expressed as

\begin{align}
\pvmiss&=-\sum_{i}^\mathrm{clustered}\vec{p}_{\mathrm{T},i}-\sum_{i}^\mathrm{unclustered}\vec{p}_{\mathrm{T},i}\\
\Rightarrow\pvmiss^\mathrm{corr.}&=\pvmiss-\sum_{i}^\mathrm{jets}\left(\, \vec{p}_{\mathrm{T},i}^\mathrm{\,full~\gls{jec}}-~\vec{p}_{\mathrm{T},i}^\mathrm{\,\gls{pu}\mbox{-}only}\right)
\end{align}
